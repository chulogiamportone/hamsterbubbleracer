...
func _on_died_body_entered(body: Node3D) -> void:
	print(body)
	print("cae")
	print(chek_point_container.get_child(body.chekpoint_number-1).get_child(0).position)
	if body.chekpoint_number:
		body.position=chek_point_container.get_child(body.chekpoint_number-1).get_child(0).position
